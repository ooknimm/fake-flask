...
class WSGIRequestHandler(StreamRequestHandler):
    ...
    def write(self, data):
        ...
        _headers_buffer.append(f"Server: WSGIServer/0.2 CPython/3.11.4\r\n".encode("latin-1"))
        timestamp = time.time()
        formated_time = email.utils.formatdate(timestamp, usegmt=True)
        _headers_buffer.append(f"Date: {formated_time}\r\n".encode("latin-1"))
